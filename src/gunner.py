...
if __name__ == '__main__':
    ...
    ret=True
    prev=ImageGrab.grab(bbox=(400,200,1200,800))
    prev= np.array(prev) #this is the array obtained from conversion
    ...
    while True:
        ret=True
        img=ImageGrab.grab(bbox=(400,200,1200,800))
        img= np.array(img) #this is the array obtained from conversion
        if ret == True:#判断视频是否结束
            gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
            flow = cv2.calcOpticalFlowFarneback(prevgray, gray, 1, 3, 15, 3, 5, 1, 0, flags=cv2.OPTFLOW_USE_INITIAL_FLOW)#Farnback光流法
